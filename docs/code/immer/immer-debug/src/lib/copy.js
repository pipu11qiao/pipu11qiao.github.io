...
export function createProxy(base, parent) {
  ...
  // 关键数据
  const state = {
    scope, // track which call this is associated with.
    modified: false, // True for both shallow and deep changes.
    finalized: false, // Used during finalization.
    assigned: {}, //track which properties have been assigned (true) or deleted (false).
    parent, // The parent draft state
    base, // The base state
    draft: null, // the base proxy
    drafts: {}, // Any property proxies
    copy: null, // the base copy with any updated values.
    revoke: null, // called by the produce function
  };
  const { revoke, proxy } = Array.isArray(base)
    ? Proxy.revocable([state], arrayTrps)
